// full/js/blocks/math.js
'use strict';

/**
 * Еталонний блок режиму «Математика» (UI).
 * - чекає завантаження math-бібліотеки;
...
function resetMathForm(scope) {
  ...
  rebuildObjectsSelect(scope, '.object1-group', '#mathCategoryObject1', '#mathObject1');
  rebuildObjectsSelect(scope, '.object2-group', '#mathCategoryObject2', '#mathObject2');
}
...
export async function initMathBlock() {
  ...
  document.addEventListener('reset', () => resetMathForm(scope));

  console.log('[mode:math] init OK');
}

// (опційно для тестів)
export { rebuildCategorySelects, rebuildObjectsSelect, resetMathForm };
