...
export async function initUniversLuminosityBlock() {
  ...
  ['lumiCategoryObject1', 'lumiCategoryObject2'].forEach(id => {
    const sel = document.getElementById(id);
    if (sel && !sel.__lumiBound) {
      sel.addEventListener('change', rebuildObjectsForSelectedCategories);
      sel.__lumiBound = true;
    }
  });
}
...
function updateCategories() {
  ...
  lib.forEach(o => {
    const has = o && o.luminosity != null && (typeof o.luminosity === 'number' || typeof o.luminosity?.value === 'number');
    if (!has) return;
  ...
  try {
    ...
    userAll.forEach(o => {
      const has = o && (typeof o.luminosity === 'number' || typeof o.luminosity?.value === 'number');
      if (!has) return;
...
function repopulateObjectsSelect(selectId, category, placeholderKey, slot) {
  ...
  lib.forEach(o => {
    const ok = o && o[`category_${lang}`] === category &&
      (typeof o.luminosity === 'number' || typeof o.luminosity?.value === 'number');
    if (!ok) return;
  ...
  try {
    ...
    user.forEach(o => {
      const ok = o && (typeof o.luminosity === 'number' || typeof o.luminosity?.value === 'number');
      if (!ok) return;
      out.push({ name: o.name || o[`name_${lang}`], source: 'user' });
    });
  } catch {}

  // пріоритет Сонця у слоті О1
  if (slot === 'object1') {
...
function rebuildForSlot(slot) {
  const catId = slot === 'object1' ? 'lumiCategoryObject1' : 'lumiCategoryObject2';
  const objId = slot === 'object1' ? 'lumiObject1' : 'lumiObject2';
  const cat = document.getElementById(catId)?.value || '';
  repopulateObjectsSelect(objId, cat, slot === 'object1' ? 'panel_placeholder_object1' : 'panel_placeholder_object2', slot);
}
