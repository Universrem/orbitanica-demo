...
/**
 * Еталонний адаптер даних для режиму «Математика».
 * Завдання:
 *  - прочитати вибір користувача з DOM (у межах контейнера режиму);
 *  - знайти об’єкти в math-бібліотеці або серед юзерських;
...
// Ключ категорії (узгоджено з blocks/*.js)
...
function readMathValueUnit(source) {
  // 1) офіційний формат математики: { quantity: { value, unit } }
  ...
  // 2) юзерський через модалку (загальний атрибут діаметра як запасний шлях)
  if (source?.attrs?.diameter && typeof source.attrs.diameter === 'object') {
    const v = Number(source.attrs.diameter.value);
    const u = norm(source.attrs.diameter.unit || source.unit || 'unit');
    if (Number.isFinite(v)) return { valueReal: v, unit: u };
  }

  // 3) інші можливі плоскі поля
  if (source && typeof source === 'object') {
    const v = Number(source.value ?? source.amount ?? source.numberValue);
    const u = norm(source.unit ?? (source.quantity && source.quantity.unit) ?? 'unit');
    if (Number.isFinite(v)) return { valueReal: v, unit: u };
  }
...
function readBaselineDiameterMeters(scope) {
  const a = scope?.querySelector('#mathBaselineDiameter, [data-field="baseline-diameter"]');
  if (a) {
 ...
 * @param {HTMLElement} scope - контейнер підсекції режиму (details#math)
 */
