// full/js/calc/calculate_money.js
'use strict';

/**
 * Еталонний калькулятор для режиму «Гроші».
 * Лише математика. Жодного DOM, i18n чи рендера.
 *
 * Правило масштабу: площа кола ∝ сумі грошей.
 * Формула для порівняння (О2 відносно О1):
 *   D2 = D1 * sqrt(V2 / V1)   ⇒   r2 = (D1/2) * sqrt(V2 / V1)
 *
 * Експорти:
 *  - setMoneyBaseline({ valueReal, unit, circleDiameterMeters, color }) → { currentScaleArea }
 *  - addMoneyCircle({ valueReal, unit, color }) → { id, scaledRadiusMeters, tooLarge, requiredBaselineMeters }
 *  - resetMoneyScale()
 *  - getMoneyScale()
 */
...
/** Поточний «масштаб площі»: скільки м² кола на 1 грошову одиницю */
 ...
 */
export function addMoneyCircle({ valueReal, unit = 'USD', color } = {}) {
...
export function resetMoneyScale() {
  __baseline = { valueReal: NaN, unit: 'USD', circleDiameterMeters: 0, color: undefined };
  __currentScaleArea = 0;
}

export function getMoneyScale() {
