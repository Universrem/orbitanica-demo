...
// Відтворення сцени «Світність» БЕЗ кліку по «Розрахувати».
// О1 і кожний О2 підставляються в селектори з локалізацією та snapshot у dataset.
// Для кожного О2 викликається onLuminosityCalculate. Перший рух центру — повне перезастосування.
...
(function registerUniversLuminosityApplier(){
  ...
  function setupFirstCenterRepaint(query) {
    ...
    const once = async () => {
      ...
      try {
        ...
        console.error('[univers_luminosity_applier] reapply after first center move failed:', e);
      }
    };
  ...
  function setSelectValue(id, value, label, snapshot) {
    ...
    if (snapshot && typeof snapshot === 'object') {
      ...
      catch (e) { console.warn('[univers_luminosity_applier] Failed to attach snapshot:', e); }
    }
  ...
  async function applyUniversLuminosityScene(query) {
    ...
    // SNAPSHOT-FIRST: потрібні О1 зі snapshot і принаймні один О2 зі snapshot
    if (!o1?.objectId || !o1?.snapshot || !o2s.length) return;

    // Підписи інфопанелі: Всесвіт → Світність
    setModeLabelKeys({ modeKey: 'panel_title_univers', subKey: 'panel_title_univers_luminosity' });

    const lang = currentLang();

    // 1) Відкрити режим, підставити О1 з локалізацією + snapshot у dataset
    ...
    setSelectValue('lumiCategoryObject1', catKey1, catLabel1);
    setSelectValue('lumiObject1',         objId1,  objLabel1, s1);
    ...
    for (const item of o2s) {
      if (!item || !item.snapshot) {
        console.warn('[univers_luminosity_applier] O2 without snapshot skipped:', item);
        continue;
      }
      ...
      if (catKey2 == null || objId2 == null) {
        console.warn('[univers_luminosity_applier] O2 lacks identifiers (category_key/id). Skipped.', item);
        continue;
      }
      ...
      setSelectValue('lumiCategoryObject2', catKey2, catLabel2);
      setSelectValue('lumiObject2',         objId2,  objLabel2, s);

      try {
        onLuminosityCalculate({ scope });
      } catch (e) {
        console.error('[univers_luminosity_applier] onLuminosityCalculate failed for O2:', item, e);
      }
