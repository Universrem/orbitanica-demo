...
(function registerUniversLuminosityApplier(){
  ...
  function setupFirstCenterRepaint(query) {
    ...
    const once = async () => {
      ...
      try {
        ...
        console.error('[univers_luminosity_applier] repaint after center change failed:', e);
      }
    };
  ...
  async function applyUniversLuminosityScene(query) {
    ...
    setNumberInput('lumiBaselineDiameter', o1.baselineDiameterMeters);
  ...
  function registerOrQueue() {
    if (window?.orbit?.registerSceneApplier) {
      ...
      (window.__orbit_pending_appliers__ ||= []).push({
        ...
        fn: applyUniversLuminosityScene
      });
    }
  }
