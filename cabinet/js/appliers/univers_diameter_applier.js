...
// Для кожного О2 викликається onDiameterCalculate. Перший рух центру — повне перезастосування.
...
(function registerUniversDiameterApplier(){
  ...
  function setupFirstCenterRepaint(query) {
    ...
    const once = async () => {
      ...
      try {
        ...
        console.error('[univers_diameter_applier] reapply after first center move failed:', e);
      }
    };
  ...
  function setSelectValue(id, value, label, snapshot) {
    ...
    if (snapshot && typeof snapshot === 'object') {
      ...
      catch (e) { console.warn('[univers_diameter_applier] Failed to attach snapshot:', e); }
    }
  ...
  async function applyUniversDiameterScene(query) {
    ...
    // Виставити підписи інфопанелі: Всесвіт → Діаметр
    ...
    setSelectValue('diamCategoryObject1', catKey1, catLabel1);
    setSelectValue('diamObject1',         objId1,  objLabel1, s1);
    ...
    for (const item of o2s) {
      if (!item || !item.snapshot) {
        console.warn('[univers_diameter_applier] O2 without snapshot skipped:', item);
        continue;
      }
      ...
      if (catKey2 == null || objId2 == null) {
        console.warn('[univers_diameter_applier] O2 lacks identifiers (category_key/id). Skipped.', item);
        continue;
      }
      ...
      setSelectValue('diamCategoryObject2', catKey2, catLabel2);
      setSelectValue('diamObject2',         objId2,  objLabel2, s);
  ...
  function registerOrQueue() {
    if (window?.orbit?.registerSceneApplier) {
      ...
      (window.__orbit_pending_appliers__ ||= []).push({ mode: 'univers_diameter', fn: applyUniversDiameterScene });
    }
  }
