...
// Для кожного О2 викликається onGeoPopulationCalculate. Перший рух центру — повне перезастосування.
...
(function registerGeoPopulationApplier(){
  ...
  function setupFirstCenterRepaint(query) {
    ...
    const once = async () => {
      ...
      try {
        ...
        console.error('[geo_population_applier] reapply after first center move failed:', e);
      }
    };
  ...
  function setSelectValue(id, value, label, snapshot) {
    ...
    if (snapshot && typeof snapshot === 'object') {
      ...
      catch (e) { console.warn('[geo_population_applier] Failed to attach snapshot:', e); }
    }
  ...
  async function applyGeoPopulationScene(query) {
    ...
    setSelectValue('geoPopCategoryObject1', catKey1, catLabel1);
    setSelectValue('geoPopObject1',         objId1,  objLabel1, s1);
    ...
    for (const item of o2s) {
      if (!item || !item.snapshot) {
        console.warn('[geo_population_applier] O2 without snapshot skipped:', item);
        continue;
      }
      ...
      if (catKey2 == null || objId2 == null) {
        console.warn('[geo_population_applier] O2 lacks identifiers (category_key/id). Skipped.', item);
        continue;
      }
      ...
      setSelectValue('geoPopCategoryObject2', catKey2, catLabel2);
      setSelectValue('geoPopObject2',         objId2,  objLabel2, s);
  ...
  function registerOrQueue() {
    if (window?.orbit?.registerSceneApplier) {
      ...
      (window.__orbit_pending_appliers__ ||= []).push({ mode: 'geo_population', fn: applyGeoPopulationScene });
    }
  }
