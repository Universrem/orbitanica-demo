...
// Серіалізатор режиму «Geography → Objects» (snapshot-first).
...
(function registerGeoObjectsSerializer() {
  ...
  function readO1FromBuffer() {
    try {
      if (typeof window?.orbit?.getGeoObjectsSelectedO1 === 'function') {
        const x = window.orbit.getGeoObjectsSelectedO1();
  ...
  function readO2ArrayFromBuffer() {
    try {
      if (typeof window?.orbit?.getGeoObjectsSelectedO2s === 'function') {
        const arr = window.orbit.getGeoObjectsSelectedO2s();
  ...
  const serializer = function serializeGeoObjectsScene() {
    // baseline керує колом; читаємо числове значення
    ...
    if (!o1 || !o2s.length) {
      console.warn('[geo_objects_serializer] Потрібні валідні О1 і принаймні один О2 зі snapshot. Виконайте «Розрахувати».');
      return null;
    }
