...
// Серіалізатор режиму «Географія → Площа» (SNAPSHOT-FIRST).
...
(function registerGeoAreaSerializer() {
  ...
  function readO1FromBuffer() {
    try {
      if (typeof window?.orbit?.getGeoAreaSelectedO1 === 'function') {
        const x = window.orbit.getGeoAreaSelectedO1();
  ...
  function readO2ArrayFromBuffer() {
    try {
      if (typeof window?.orbit?.getGeoAreaSelectedO2s === 'function') {
        const arr = window.orbit.getGeoAreaSelectedO2s();
  ...
  const serializer = function serializeGeoAreaScene() {
    ...
    if (!o1 || !o2s.length) {
      console.warn('[geo_area_serializer] Потрібні валідні О1 і принаймні один О2 зі snapshot. Виконайте «Розрахувати».');
      return null;
    }
