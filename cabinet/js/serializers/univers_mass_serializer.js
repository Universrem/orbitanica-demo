...
// Серіалізатор режиму «Universe → Mass».
...
(function registerUniversMassSerializer() {
  ...
  function readO1FromBuffer() {
    try {
      if (typeof window?.orbit?.getUniversMassSelectedO1 === 'function') {
        const x = window.orbit.getUniversMassSelectedO1();
  ...
  function readO2ArrayFromBuffer() {
    try {
      if (typeof window?.orbit?.getUniversMassSelectedO2s === 'function') {
        const arr = window.orbit.getUniversMassSelectedO2s();
  ...
  const serializer = function serializeUniversMassScene() {
    // baseline керує колом; читаємо числове значення з #massCircleObject1
    ...
    if (!o1 || !o2s.length) {
      console.warn('[univers_mass_serializer] Потрібні валідні О1 і принаймні один О2 зі snapshot. Виконайте «Розрахувати».');
      return null;
    }
