// /cabinet/js/serializers/univers_mass_serializer.js
// Серіалізатор режиму «Universe → Mass» з підтримкою масиву О2 (o2s).
// Працює незалежно від порядку завантаження: якщо місток ще не готовий — стає в чергу.

import { getCurrentLang } from '/js/i18n.js';

(function registerUniversMassSerializer() {
  'use strict';
  ...
  function readO2Array() {
    try {
      if (typeof window?.orbit?.getUniversMassSelectedO2s === 'function') {
        const fromState = window.orbit.getUniversMassSelectedO2s();
        if (!Array.isArray(fromState)) {
          console.warn('[univers_mass_serializer] getUniversMassSelectedO2s() must return array; got:', typeof fromState);
        } else {
      ...
      console.warn('[univers_mass_serializer] Error reading O2 from state:', err);
    }

    // Фолбек: поточний вибір у випадайках → масив із 1 елемента або порожній
    const cat2 = readSelectInfo('massCategoryObject2');
    const obj2 = readSelectInfo('massObject2');
    const one = normO2({ categoryKey: cat2.value, objectId: obj2.value, name: obj2.label });
    return one ? [one] : [];
  }

  // ---------- serializer ----------
  const serializer = function serializeUniversMassScene() {
    // О1
    const cat1 = readSelectInfo('massCategoryObject1');
    const obj1 = readSelectInfo('massObject1');
    const baselineMeters = readNumberOrNull('massCircleObject1');
    ...
    if (!obj1.value || o2s.length === 0) {
      console.warn('[univers_mass_serializer] Incomplete data: need O1 and at least one O2');
      return null;
    }
  ...
  try {
    if (window?.orbit?.registerSceneSerializer) {
      window.orbit.registerSceneSerializer('univers_mass', serializer);
    } else {
    ...
    console.error('[univers_mass_serializer] register failed:', e);
  }
})();
