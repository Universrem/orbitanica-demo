...
// Серіалізатор режиму «Математика» (snapshot-first).
...
(function registerMathSerializer() {
  ...
  function readO1FromBuffer() {
    try {
      if (typeof window?.orbit?.getMathSelectedO1 === 'function') {
        const x = window.orbit.getMathSelectedO1();
  ...
  function readO2ArrayFromBuffer() {
    try {
      if (typeof window?.orbit?.getMathSelectedO2s === 'function') {
        const arr = window.orbit.getMathSelectedO2s();
  ...
  const serializer = function serializeMathScene() {
    ...
    if (!o1 || !o2s.length) {
      console.warn('[math_serializer] Потрібні валідні О1 і принаймні один О2 зі snapshot. Виконайте «Розрахувати».');
      return null;
    }
