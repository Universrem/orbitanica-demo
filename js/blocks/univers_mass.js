...
function rebuildObjectsSelect(scope, groupSelector, catSelector, objSelector) {
  ...
  // офіційні об’єкти цієї категорії (лише з масою)
...
function resetMassForm(scope) {
  ...
  rebuildObjectsSelect(scope, '.object1-group', '#massCategoryObject1', '#massObject1');
  rebuildObjectsSelect(scope, '.object2-group', '#massCategoryObject2', '#massObject2');
}
...
export async function initUniversMassBlock() {
  ...
  document.addEventListener('reset', () => resetMassForm(scope));

  console.log('[mode:mass] init OK');
}

// (опційно для тестів)
export { rebuildCategorySelects, rebuildObjectsSelect, resetMassForm };
